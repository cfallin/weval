...
# Borrowed from Wizer. Original description follows:
#
...
bin_pkgname=weval-$tag-$platform
...
if [ "$platform" = "x86_64-windows" ]; then
  cp target/release/weval.exe tmp/$bin_pkgname
  fmt=zip
elif [ "$platform" = "x86_64-mingw" ]; then
  cp target/x86_64-pc-windows-gnu/release/weval.exe tmp/$bin_pkgname
  fmt=zip
elif [ "$target" = "" ]; then
  cp target/release/weval tmp/$bin_pkgname
else
  cp target/$target/release/weval tmp/$bin_pkgname
fi
